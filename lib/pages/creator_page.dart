...
import 'package:flutter/material.dart';
import 'package:http/http.dart' as http show post;
import 'home_page.dart'; // ✅ Import your HomePage for navigation
...
class _CreatorPageState extends State<CreatorPage> {
  ...
  void _register() async {
    ...
    final url = Uri.parse('https://lovecalculator-mu.vercel.app/api/creator/register');
    final response = await http.post(
      url,
      headers: {'Content-Type': 'application/json'},
      body: jsonEncode({'name': name}),
    ...
    if (response.statusCode == 201) {
      final data = jsonDecode(response.body);
      final newCreatorId = data['id'];
      final newCreatorName = data['name'];

      // ✅ Navigate to HomePage with creatorId
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(
          builder: (context) => HomePage(creatorId: newCreatorId),
        ),
      );

      // 🔁 Optional: setState if you want to show dashboard before navigating
      setState(() {
  ...
  Widget build(BuildContext context) {
    if (!_isRegistered) {
      ...
      // Not reached in normal flow due to pushReplacement, but kept for fallback
      return Scaffold(
        appBar: AppBar(title: const Text('Creator Dashboard')),
        body: Padding(
          padding: const EdgeInsets.all(16),
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 20),
              const Text('Share this link with your friends:'),
              SelectableText(
                'https://lovecalculator-mu.vercel.app/?creator_id=$creatorId',
                style: const TextStyle(
                  fontSize: 16,
                  color: Colors.blue,
                  decoration: TextDecoration.underline,
